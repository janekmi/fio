/*
 * librpma I/O engine - client part
 *
...
/* XXX: for server version of the engine
struct fio_librpmaio_server_options {
	struct thread_data *td;
	char *listen_ip;
...
static struct fio_option options[] = {
	{
		.name	= "listen_ip",
		.lname	= "librpma engine listen ip",
		.type	= FIO_OPT_STR_STORE,
		.off1	= offsetof(struct fio_librpmaio_server_options, listen_ip),
	...
	{
		.name	= "listen_port",
		.lname	= "librpma engine listen port",
		.type	= FIO_OPT_INT,
		.off1	= offsetof(struct fio_librpmaio_server_options, listen_port),
		.minval	= 1,
		.maxval	= 65535,
